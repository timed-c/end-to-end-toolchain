...
void write_workload(FILE* fp, int randnum, char* bmark, int knd){
    if(knd == 0){
        fprintf(fp, "\t \t func(%d);\n", randnum);
    }
    else{
...
void main(int argc, char *argv[]){
    ...
    int count = 0;
    if(argc < 8){
        printf("Error: input parameters missing \nExpected input parameters are number-of-task, no-of-frames, range-of-offset, (small, large, mix), fname, seed, and kind\n");
        return;
    }
    task = atoi(argv[1]);
    frame = atoi(argv[2]);
    ...
    for(i=0; i<tsk; i++){
        ...
        for(j=0; j<frme; j++){
           ...
           // printf("%d %d\n", this_period, frame_period);
            ...
            randnum = rand()% MBENCH;
            write_workload(fp, (randnum+1), bmark, knd);
            if(j != (frme-1)){
                write_spolicy(fp, frame_period);
            }
            else{
