...
import os
import sys
from interface_for_kta_tool import *
...
def get_wcet_otawa(cf, filename):
    ...
    fnfp.write("\n}")
    fnfp.close()
    exe="arm-none-eabi-gcc --specs=nosys.specs wcet_main.c -o wcet_main" 
    print(exe)
    ...
    exe="mkff wcet_main > wcet_main.ff"
    op= subprocess.Popen(exe, shell=True, stdout=subprocess.PIPE).stdout.read()
    print(exe)
    raw_input("Please modify the flow file and PRESS ENTER TO CONTINUE")
    #op= subprocess.Popen(exe, shell=True, stdout=subprocess.PIPE).stdout.read()
    exe="owcet -s generic wcet_main"
    print(exe)
    op= subprocess.Popen(exe, shell=True, stdout=subprocess.PIPE).stdout.read()
    print(op)
    oplist=op.split(" ")
        ...
        if is_timingpoint(task[i]):
            ...
            if clist[0]!=" }\n":
                ...
                if (hard):
                    ...
                    if (wcet_tool == "KTA"):
                        tW=get_wcet_kta(clist,fname)
                    else:
                        tW=get_wcet_otawa(clist,fname)
                    hset.append(tW)
                    #fn_fp.close() 
                f=[tid, tB, tW, clist]
                fset.append(f)
